...
export default function AdminPage() {
  const [activeTab, setActiveTab] = useState<"products" | "gas" | "solar">(
    "products",
  );
  ...
  const handleUpdateGasPrice = async (newPrice: number) => {
    try {
      await apiJson("/api/gas-price", "PUT", { price: newPrice });
      await fetchData();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSaveSolar = async (data: Partial<SolarProject>) => {
    try {
      ...
      const url = editingSolar
        ? `/api/solar-projects?id=${editingSolar.id}`
        : "/api/solar-projects";
      await apiJson(url, method, data);
      await fetchData();
    } catch (err) {
  ...
  const handleDeleteSolar = async (id: string) => {
    if (!confirm("Delete project?")) return;
    try {
      await apiJson(`/api/solar-projects?id=${id}`, "DELETE");
      await fetchData();
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6 border-b">
        <button
          ...
          className={`px-6 py-3 font-semibold border-b-2 ${activeTab === "products" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-600"}`}
        >
          <ShoppingBag className="inline h-5 w-5 mr-2" /> Products
        </button>
        <button
          onClick={() => setActiveTab("gas")}
          className={`px-6 py-3 font-semibold border-b-2 ${activeTab === "gas" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-600"}`}
        >
          <Droplets className="inline h-5 w-5 mr-2" /> Gas Price
        </button>
        <button
          onClick={() => setActiveTab("solar")}
          className={`px-6 py-3 font-semibold border-b-2 ${activeTab === "solar" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-600"}`}
        >
            ...
            {products.map((p) => (
              <div key={p.id} className="bg-white rounded-lg border p-4">
                ...
                <div className="flex gap-2 text-xs mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded ${p.stock === "in-stock" ? "bg-emerald-100 text-emerald-800" : p.stock === "low-stock" ? "bg-orange-100 text-orange-800" : "bg-red-100 text-red-800"}`}
                  >
                    {p.stock}
                  </span>
                </div>
          ...
          <div className="bg-white rounded-lg border p-6">
            ...
            <input
              ...
              onBlur={(e) => {
                ...
                if (newPrice !== gasPrice.price) handleUpdateGasPrice(newPrice);
              }}
